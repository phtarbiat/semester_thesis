function [x_out,numIterations,errorFlag,normFun,numFunEva,numJacEva] = columnUpdating(fun,jac,x_0,errorMargin,maxIteration)
% Implementation of the Column-Updating-Method
%
...
% Check if error in the mert-function is low enough to exit iteration
...
for nIterations = 1:maxIteration
    ...
    e_k = zeros(n,1);
    e_k(idx_k) = 1;
    H_k = H_k + ((s_k - H_k*y_k) * (e_k' * H_k)) / (e_k' * H_k * y_k);
